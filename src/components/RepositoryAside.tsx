...
import { IDirectory, IFolder, ITextFile, PushToRepositoryData, TextFileDTO } from '../interfaces/dto';
import { useCodeSharingHub, useUser } from '../hooks';
import { Directories, Github, Users } from '../api';
import { createFile } from '../codeshare/socket';
...
const RepositoryAside: React.FC<RepositoryAsideProps> = ({ repository, selectFile, setFiles, files }) => {
  ...
  useEffect(() => {
    ...
    codeHub.onCreateFile((file) => {
      ...
      createFile(repository.id, newFile)
    });
  ...
  }, [repository.id, codeHub, files, updateDirectory, selectedFile, selectFile, setFiles]);
